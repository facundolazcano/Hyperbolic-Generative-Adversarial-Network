...
from utils.utils_dataset import loader_CIFAR10, sample_data, loader_LSUN
...
def train(args, loader, generator, discriminator, g_optim, d_optim, g_ema, device, writer):
    ...
    for idx in pbar:
        ...
        if args.dataset == 0:
            ...
            if shape[0]!= args.batch_size or shape[1]!=3 or shape[2]!=256 or shape[3]!=256:
                print('error:', shape)
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=int, default='0:CIFAR10,  1:lsun-cats')
    ...
    generator = Generator(
        ...
        cfg=args.cfg, c=args.c, activation_mix=activation_mix
    ).to(device)
    ...
    if args.dataset == 0:
        ...
        loader = loader_LSUN(batch_size=args.batch_size, distributed=args.distributed)
