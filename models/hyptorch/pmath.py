...
def _expmap0(u, c):
    sqrt_c = c ** 0.5
    #u_norm = torch.clamp_min(u.norm(dim=-1, p=2, keepdim=True), 1e-5)
    epsilon = torch.as_tensor(1e-6)
    u_norm = u.norm(dim=-1, p=2, keepdim=True) + epsilon
...
def _logmap0(y, c):
    ...
    epsilon = torch.as_tensor(1e-6)
    y_norm = y.norm(dim=-1, p=2, keepdim=True) + epsilon
...
def mobius_scalar_mult(u, r, *, c=1.0):
    r"""
    Exponential map for Poincare ball model from :math:`0`.
    .. math::
        \operatorname{Exp}^c_0(u) = \tanh(\sqrt{c}/2 \|u\|_2) \frac{u}{\sqrt{c}\|u\|_2}
    ...
    """
    c = torch.as_tensor(c).type_as(u)
    r = torch.as_tensor(r).type_as(u)
    return _mobius_scalar_mult(u, r, c)


def _mobius_scalar_mult(u, r, c):
    ...
    epsilon = torch.as_tensor(1e-6)
    u_norm = u.norm(dim=-1, p=2, keepdim=True) + epsilon
    gamma_1 = tanh(artanh(sqrt_c * u_norm) * r) * u / (sqrt_c * u_norm)
